...
class AbstractSubsetImgPredictor(AbstractPredictor):
    ...
    def getImage(self):
        if self.IMG_SCALE < 1.0:
            self.output_data.image_data.image_np =  cv2.resize(self.SRC_DATA.image_data.image_np.copy(),
                                                (0,0), fx=self.IMG_SCALE, fy=self.IMG_SCALE)
        else:
            self.output_data.image_data.image_np = self.SRC_DATA.image_data.image_np
    ...
    def getImgSubset(self, bb, image_np):
        ...
        if fixed_bb[0] == fixed_bb[2] or fixed_bb[1] == fixed_bb[3]:
            cropped_img = None
            print('Subset Image Predictor found a null image.')
        return [fixed_bb, cropped_img]
    def remap_bb_coords(self, cropped_img, full_size_image, bbs, scores, classes,_ids, fixed_bb):
        '''
        ...
        for remapped_bb, score, class_,_id in zip(bbs, scores, classes,_ids):
            [h,w,_] = full_size_image.shape
            if remapped_bb[0] > 0.0 and score > self.output_data.score_thresh:
        ...
        return [output_bbs, output_scores, output_classes,output_ids]
